...
- **`reach-runtime` is retired rather than rehomed, and runtime state is session-scoped.** On the new contract a plugin's editable surface is its OWN `Config`, addressed by the profile entry id and projected field by field through `.volatile()`; a runtime namespace has no replacement. The fields that namespace held (`security.owner`/`allowFrom`, chat↔session and chat↔cwd mappings, delivered bookkeeping, the audit tail, and four runtime toggles) are plugin-private bookkeeping written by the plugin itself — never form-editable, and never read by the shipped settings tab, which goes through the `reach` Remote service — so promoting them to volatile `Config` fields would have invented an editable surface that never existed, and moving them into `ctx.storageDomain` would have bought durability at the price of a new hard storage dependency in a plugin whose whole design is zero hard dependencies. **User-visible consequence, stated plainly: owner pairing, chat/workspace bindings, delivery bookkeeping and the audit tail no longer survive a restart, and the `silent`/`notify`/`tasks`/`queueMode` switches fall back to the row config on restart.** What must outlive a restart already lives elsewhere — channel tokens in `ctx.credentials`, configuration in the profile patch. The removed `RuntimeStateSchema` export went with the namespace; it described a contract no host accepts.
- The A7-② ordering hazard is gone rather than fixed: registering the settings face **before** the channel adapters were constructed used to strand two registered namespaces when a constructor threw. There are no namespaces to strand now, and the pin that replaces it asserts the surviving half of the invariant — a constructor throw leaves the page policy unclaimed.
- **The gate that should have caught this, and did not.** The test fixtures handed the plugin a fabricated settings service that offered `register` and a writable scope, so `pnpm test` stayed green while every real profile failed to mount, and `typecheck` could not see it because `ctx.get('settings')` returns `any` and the old call site asserted it away with `as SettingsFace`. The fixture (`tests/fixtures/settings.ts`) now carries ONLY the real `SettingsForms` surface — `configure`, no `register` — and `tests/settings-face.spec.ts` compares it against the installed package instead of against memory: the real class must not carry `register`, every key of the fake must exist on the real class, a `register` call against the fake must throw the way a real profile does, and the plugin mounted with a REAL `SettingsForms` instance must reach `configure`. Replayed against the pre-fix `apply`, that gate fails four tests across three files.
- The CI keyless smoke could not see a dead row either: a plugin that fails to activate does not stop the harness from booting, and the `llm-deepseek` row answers `MISSING_CREDENTIAL` regardless, which the smoke treated as proof that the tree had loaded. The Compat smoke now fails on any `did not activate` warning or `TypeError` in its log before it evaluates that signal.
...
- The retired A7 regressions are replaced rather than re-pointed: `reach-runtime` write failures and namespace registration ordering described surfaces that no longer exist, so the suite now pins what took their place — degrade on a foreign settings service, claim the page policy on the plugin's own fiber exactly once, claim nothing when a constructor throws, and seed runtime state from the row config.
