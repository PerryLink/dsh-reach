...
[![dshfind](https://dshfind.com/api/badge/PerryLink/dsh-reach?metric=downloads&lang=pt)](https://dshfind.com/pt/plugins/PerryLink/dsh-reach?ref=badge)
...
> **Status: Fases 1–3 concluídas (canais WeChat + Telegram + Feishu, v0.1.10); bases de canais v2 (QQ/DingTalk/WeCom) sobre o registro aberto `reachChannels`.**
...
| Superfície | Status |
|---|---|
| Harness | DeepSeek Harness **dsh-v0.1.7-alpha.1** (tag do GitHub). Faixa de peers `>=0.1.2-rc.1 <0.2.0 \|\| >=0.1.5-alpha.1 <0.2.0 \|\| >=0.1.6-0 <0.2.0 \|\| >=0.1.7-0 <0.2.0`; os pins de dev/test ficam de propósito na linha publicada `0.1.7-alpha.2`, que é a face medida pela régua `typecheck:ci` (a régua `typecheck` mede o checkout através dos `paths` do `tsconfig.json`). Verificado em 2026-09-18 com a cadeia completa de gates; o smoke de bare-import + perfil scratch + keyless roda no workflow Compat em cada PR. |
| Node | `^22.19.0 \|\| >=24.0.0` |
...
- Console de sessão (`/reach /help /status /silent /notify /tasks /enter /workspace /session /preset /model /perm /history /stop /next`) e aba de configurações.
