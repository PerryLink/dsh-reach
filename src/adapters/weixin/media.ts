 ...
 *
 * Ported from pan17/dsh-wechat 0.7.2 `dist/weixin/media.js` (MIT), which
 * adapts @tencent-weixin/openclaw-weixin cdn/aes-ecb.ts and
...
/**
 * Parse the AES key from a CDN media reference. The key is base64 and can
 * decode to either 16 raw bytes (used directly) or 32 ASCII hex chars.
 */
export function parseAesKey(media: { readonly aes_key?: string }): Buffer | null {
...
export async function downloadAndDecrypt(
  ...
  if (filekey) url += `&filekey=${encodeURIComponent(filekey)}`
