...
// declarations (lib/types/**/*.d.ts). TypeScript's
// `rewriteRelativeImportExtensions` rewrites the JS emit but NOT the
// declaration emit, so declaration consumers under NodeNext resolution would
// fail on `.ts` specifiers. Node builtins only: this runs inside the
// git-install `prepare` with production deps alone.
